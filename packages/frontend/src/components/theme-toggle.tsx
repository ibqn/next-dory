...
const RadioGroup = ({ className, ...props }: RadioGroupProps) => {
  return <div {...props} className={cn("", className)}></div>
}
...
const Radio = ({ className, ...props }: RadioProps) => {
  ...
  return (
    <div
      {...props}
      className={cn(
        "rounded-full bg-white p-1.5 ring inset-ring ring-gray-950/10 inset-ring-white/10 *:size-7 sm:p-0 dark:bg-gray-700 dark:text-white dark:ring-transparent",
...
function ThemeToggleButton({ value, className, ...props }: ComponentProps<"button"> & { value: string }) {
  ...
  return (
    <button
      ...
      className={cn("z-10 cursor-pointer rounded-full p-1.5 *:size-7 sm:p-0", className)}
    />
  )
}
...
export const ThemeToggle = () => {
  return (
    <RadioGroup className="relative z-0 inline-grid grid-cols-3 gap-0.5 rounded-full bg-gray-950/5 p-0.75 text-gray-950 dark:bg-white/10 dark:text-white">
      <Radio className="absolute top-1/2 -translate-y-1/2">
        <div></div>
      </Radio>
